"""
INSTRUMENT 1 - PARAMETRIC LOOK-AHEAD BIAS (OOS DECAY)
=====================================================
...
K_MDE = 2.927                     # z(.95) + z(.90): TOST, 80% power, alpha=.05
ALPHA_LEVEL = 0.05
...
def main():
    ...
    print("\n[2/6] building paired differentials")
    ctrl_mean = ctrl_ic.mean(axis=1, skipna=True).rename("ctrl_mean")
    ctrl_mean = ctrl_mean.mask(ctrl_ic.notna().sum(axis=1) < 10)
    diff, betas, rhos = build_differentials(trt_ic, ctrl_mean, cutoff)
    loo = loo_control_differentials(ctrl_ic, cutoff)

    sig_raw = float(trt_ic.std(ddof=1).median())
    sig_d = float(diff.std(ddof=1).median())
    print(f"   median beta {betas.median():+.3f} | median rho {rhos.median():+.3f}")
    print(f"   sigma raw {sig_raw:.4f} -> paired {sig_d:.4f}"
          f"  ({100*(1-sig_d/sig_raw):+.1f}% variance change)")
    ...
    print(f"\n   frontier at L={L}: IS Rank IC must exceed "
          f"{frontier_ic(sig_d, r, effective_T(pooled.iloc[-L:])):.4f} per alpha, "
          f"or {frontier_ic(sig_d, r, effective_T(pooled.iloc[-L:]), N_eff):.4f} pooled")
