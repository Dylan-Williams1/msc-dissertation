...
PROVIDER CONSTRAINT — NO SEED.
The Messages API exposes no seed parameter, so this corpus is not replayable
call-by-call. A per-call seed is still derived and recorded as
`intended_call_seed` to keep the call plan auditable, but the mandatory `seed`
field is null, because recording a seed that was never applied would be false
provenance. Note also that no provider guarantees determinism even at
temperature 0; reproducibility is best-effort.

Usage:
    pip install anthropic python-dotenv
    python alph_gen_code_claude.py --dry-run     # plan only, spends nothing
...
API_CONSTRAINTS = {
    "seed_exposed": False,
    "seed_note": (
        "The Messages API exposes no seed parameter. This corpus is not "
        "replayable call-by-call. `intended_call_seed` records the planned "
        "value; `seed` is null because none was applied."
    ),
    "temperature_supported": True,
    "temperature_range": "0.0-1.0",
    "reasoning_control": "output_config.effort",
    "determinism_note": (
...
API_CONSTRAINT_NOTES = [
    f"temperature {TEMPERATURE} applied (supported range 0.0-1.0)",
    f"reasoning depth via output_config.effort={REASONING_EFFORT}",
    "NO SEED — not replayable call-by-call",
]
...
#
# Anthropic publishes TWO dates per model, and the distinction is exactly the
...
MODEL_KNOWLEDGE_CUTOFF_UPPER = "2026-05"     # training data cutoff
...
def build_client():
    ...
    # CLAUDE_API_KEY first, ANTHROPIC_API_KEY as a fallback: the SDK itself reads
    # only the latter, so accepting both avoids a silent mismatch.
    key = os.environ.get(API_KEY_ENV_VAR) or os.environ.get("ANTHROPIC_API_KEY")
    if not key:
        raise SystemExit(
            f"{API_KEY_ENV_VAR} not set. Put {API_KEY_ENV_VAR}=<your_key> in a "
            ".env file next to this script (ANTHROPIC_API_KEY is also accepted). "
            "If you are using a .env file, `pip install python-dotenv` — without "
            "it the file is never read and the key will look unset."
        )
    # Passed explicitly: the SDK would otherwise look only at ANTHROPIC_API_KEY
    # and fail even though the key was found here.
...
def call_model(client, model, user_prompt, call_seed):
    ...
    applied = {
        ...
        "seed_note": "no seed parameter exists on this API",
    ...
    attempts = [
        ("temperature+effort", dict(temperature=TEMPERATURE,
                                    output_config={"effort": REASONING_EFFORT})),
        ("effort_only", dict(output_config={"effort": REASONING_EFFORT})),
        ("temperature_only", dict(temperature=TEMPERATURE)),
        ("bare", dict()),
    ]
    ...
    for label, extra in attempts:
        try:
            ...
            msg = str(exc).lower()
            last_exc = exc
            if any(t in msg for t in ("temperature", "output_config", "effort",
                                      "unsupported", "invalid_request")):
                applied["fallbacks"].append(f"{label}: API rejected ({str(exc)[:200]})")
                continue
            raise

        applied["temperature"] = extra.get("temperature")
        applied["reasoning_effort"] = (extra.get("output_config") or {}).get("effort")
        ...
        tool_blocks = [
            ...
            if getattr(b, "type", None) not in ("text", "thinking", "redacted_thinking")
        ]
        ...
        audit = {
            ...
            "grounding": {
                "grounding_metadata_present": bool(tool_blocks),
                "detail": str(tool_blocks)[:2000] if tool_blocks else None,
            },
